...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { response: metricRes, payload: metricPayload } = await fetchAirbridgeJson(
      `https://api.airbridge.io/dataspec/v2/apps/${encodeURIComponent(appName as string)}/actual-report/metrics`,
      { method: "GET" },
      tokenForReport,
    );
    if (!metricRes.ok) {
      ...
      if (!fieldRes.ok) {
        ...
        if (reportMetricKeys.length === 0 || !linkDimension) {
          ...
          if (activeTaskId && reportStatus === "UNAVAILABLE" && reportMessage) {
            // no-op: keep behavior for hard failures above
          } else if (activeTaskId) {
            let reportPayload: unknown = null;
            let finalStatus: AirbridgeTask["status"] = "PENDING";
            const maxPoll = requestedTaskId ? 1 : 6;
            ...
            if (finalStatus === "SUCCESS") {
              ...
              // Fallback: short-link dimension 매칭 실패 시 캠페인 파라미터 기준으로 한 번 더 조회한다.
              ...
              if (rawMessage.toLowerCase().includes("requested url was not found")) {
                ...
                if (finalStatus === "FAILURE" || finalStatus === "CANCELED") {
