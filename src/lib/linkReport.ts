...
async function getReportMeta(appName: string, tokenForReport: string): Promise<ReportMeta> {
  ...
  if (!metricRes.ok) {
    throw new Error(
      toRecord(metricPayload)?.detail?.toString() ||
        toRecord(metricPayload)?.title?.toString() ||
  ...
  if (!fieldRes.ok) {
    throw new Error(
      toRecord(fieldPayload)?.detail?.toString() ||
        toRecord(fieldPayload)?.title?.toString() ||
...
export async function getLinkReport(params: {
  shortUrl?: string;
  airbridgeLinkId?: string;
  requestedTaskId?: string;
  forceRefresh?: boolean;
}): Promise<LinkReportData> {
  const shortUrl = params.shortUrl?.trim() ?? "";
  const airbridgeLinkId = params.airbridgeLinkId?.trim() ?? "";
  const requestedTaskId = params.requestedTaskId?.trim() ?? "";
  const forceRefresh = params.forceRefresh === true;

  if (!shortUrl && !airbridgeLinkId) {
    throw new Error("short_url 또는 airbridge_link_id가 필요합니다.");
  }

  if (shortUrl && !forceRefresh) {
    const cachedData = await readCachedReport(shortUrl);
    if (cachedData) {
      return cachedData as LinkReportData;
    }
  }
  ...
  if (isTemplateValue(appName) || tokens.length === 0) {
    throw new Error(
      "AIRBRIDGE_APP_NAME, AIRBRIDGE_TRACKING_LINK_API_TOKEN(또는 AIRBRIDGE_API_TOKEN) 확인",
    );
  }
  ...
  if (!identifier) {
    throw new Error("short_url에서 shortId를 추출할 수 없습니다.");
  }
  ...
  for (const token of tokens) {
    ...
    detailErrorMessage =
      toRecord(payload)?.detail?.toString() ||
      toRecord(payload)?.title?.toString() ||
      toRecord(payload)?.message?.toString() ||
      `status ${detailResponse.status}`;
  }

  if (!detailSuccess) {
    throw new Error(`Airbridge tracking link 조회 실패: ${detailErrorMessage}`);
  }
  ...
  let reportMetrics: LinkReportData["report_metrics"] = {
    clicks: null,
    impressions: null,
    app_installs: null,
  ...
  let reportDimensions: LinkReportData["report_dimensions"] = {
    channel_type: null,
    channel: null,
    campaign: null,
    ad_group: null,
  ...
  try {
    ...
    const meta = await getReportMeta(appName as string, tokenForReport);
  ...
  const responseData: LinkReportData = {
    idType,
    identifier,
    ...
    tracking_link: {
      id: (() => {
        const rawId = toRecord(toRecord(detailPayload)?.data)?.id;
